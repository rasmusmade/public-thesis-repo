...
'''
Importing segmentation pipelines
'''
...
def current_pipeline():
    '''
    HTTP endpoint that returns which pipeline is currently running.
    '''
...
def video_feed():
    '''
    HTTP endpoints that yields frames depending on the active pipeline.
    '''
    def stream():
        SLEEP = 1/30
        while True:
            if activePipeline == "mediapipe":
                img = MediapipePipeline.get_latest_output_image()
                if img is None:
...
def click():
    """
    Endpoint to receive click events from the frontend.
    """
...
if __name__ == "__main__":
    # Startin Flask app on port 8000
